...
<div class="content"> <div class="box box-info">提示：你可以编辑内容和分类 </div><a href="<?php echo url('article/lists',array('cid'=>$article['cid']) );?>" class="btn btn-blue">列 表</a>
  <form action="<?php echo url('article/edit');?>" method="post" enctype="multipart/form-data" id="articleform">
  	<p>
    ...
    <p>
      ...
      <select name="cid" id="cid">        
        <?php $n=1;if(is_array($cattree)) foreach($cattree AS $val) { ?>
            <option value="<?php echo $val['id']; ?>" <?php if($article['cid']==$val['id']) { ?>selected<?php } ?>><?php echo $val['ctitle']; ?></option>
        <?php $n++;}unset($n); ?>
      </select>
    </p>
    <p>
      <label for="content" class="required">内容</label>      <br>
      <textarea id="content" name="content" class="full large" ><?php echo $article['content']; ?></textarea>
...
<script type="text/javascript">
    ...
    <p>
      ...
      <input type="text" name="keywords" id="keywords" class="half"  value="<?php echo $article['keywords']; ?>">
    </p>
    <p>
      <label for="description">摘要</label><br>
      <textarea type="text" name="description" id="description" class="half small"><?php echo $article['description']; ?></textarea>
    </p>
    <p>
