...
class Model extends cpModel {
	...
	private   $tableField       =   false;	  // 是开启表结构过滤
	...
	function __construct(){	
	    $config = cpConfig::$config['APP'];//取得设置后的配置信息		
		parent::__construct($config);//调用cpModel构造方法
		
		$suffix = explode('.',cpConfig::get('MODEL_SUFFIX'));//取得后缀
     ...
     * @return mixed
     */
     public function checkData($data='',$token=true) {     	
        // 如果没有传值默认取POST数据
        if(empty($data)) {
            $data   =   $_POST;
        ...
        if ($token && cpConfig::$config['APP']['TOKEN_NAME']){
    ...
    public function autoCheckToken($data) {     	    
        ...
        $name = cpConfig::$config['APP']['TOKEN_NAME'];
        if ($data[$name]==$token){        	
        	return true;
        }else{
        	return false;
    ...
    static public function Token_val() { 
    	$authkey = cpConfig::$config['APP']['authkey'];
