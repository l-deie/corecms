...
/*		
		$page=new Page();
		$page->pageSuffix='.html';
		$listRows=40;//每页显示的信息条数
		$cur_page=$page->getCurPage();
		$limit_start=($cur_page-1)*$listRows;
		$limit=$limit_start.','.$listRows;
		
		$pagestring = $page->show(url('index/index',array('page'=>'{page}')),500,$listRows,10,4);
*/
class Page 
 {		
...
private function _requestUri()
{
 ...
 {
 	//原生url处理
 	if (cpConfig::get('URL_REWRITE_ON')==2 || cpConfig::get('URL_REWRITE_ON')==3) {
 		$this->_gmaUrl($url);
 		return ;
 	}
