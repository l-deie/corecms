...
//设置时区
function timezone_set($timeoffset = 'PRC'){
...
function url($module,$request='',$author=''){	
	return Murl::url($module,$request,$author);
}
...
function  module($group_module){
	...
	if(!isset($config['MODULE_PATH'])){
		$config['CONFIG_PATH'] = cpConfig::get('CONFIG_PATH');
	...
	//分组处理
	list($group,$module) = explode('/',$group_module);
	if(empty($module)){//只有一个参数，没有$module
		$module = $group;
		$group = CP_GROUP;//默认当前分组
	}else{//$group,$module都有值 ，说明指定分组时
		$group_config_file = $config['CONFIG_PATH'] . $group . '.php';
		$group_config = array();//初始化变量防止E_NOTICE报错
		file_exists($group_config_file) && $group_config = include($group_config_file);
		!empty($group_config) && $config = array_merge($config, $group_config);//参数配置
	}

	$module_file = $config['MODULE_PATH'].$module.$config['MODULE_SUFFIX'];
	if(file_exists($module_file)){
		require_once($module_file);//加载模型文件
		$classname=$module.$config['MODULE_CLASS_SUFFIX'];
		if(class_exists($classname)){
			return  $module_obj[$group_module]=new $classname();
		}
	}else{
		return false;
	}
...
function cookie($name, $value='', $option=null) {
	// 默认设置
	$config = array(
	'prefix'    =>  cpConfig::get('COOKIE_PREFIX'), // cookie 名称前缀
	'expire'    =>  cpConfig::get('COOKIE_EXPIRE'), // cookie 保存时间
	'path'      =>  cpConfig::get('COOKIE_PATH'), // cookie 保存路径
	'domain'    =>  cpConfig::get('COOKIE_DOMAIN'), // cookie 有效域名
	);
	// 参数设置(会覆盖黙认设置)
	if (!is_null($option)) {
